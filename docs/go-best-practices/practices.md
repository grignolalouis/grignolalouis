...
**Note:** This is part of a series of documents that outline Go Style at Google. This document is **neither normative nor canonical**, and is an auxiliary document to the core style guide. See the [overview](./index.md) for more information.
...
See the [overview](./index.md) for the full set of Style Guide documents.
...
Go style is flexible about file size, because maintainers can move code within a package from one file to another without affecting callers. But as a general guideline: it is usually not a good idea to have a single file with many thousands of lines in it, or having many tiny files. There is no "one type, one file" convention as in some other languages. As a rule of thumb, files should be focused enough that a maintainer can tell which file contains something, and the files should be small enough that it will be easy to find once there. The standard library often splits large packages to several source files, grouping related code by file. The source for [package `bytes`](https://cs.opensource.google/go/go/+/master:src/bytes/) is a good example. Packages with long package documentation may choose to dedicate one file called `doc.go` that has the package documentation, a package declaration, and nothing else, but this is not required.
...
- small packages that contain one cohesive idea that warrant nothing more being added nor nothing being removed:
  - [package `csv`](https://pkg.go.dev/encoding/csv): CSV data encoding and decoding with responsibility split respectively between `reader.go` and `writer.go`.
  - [package `expvar`](https://pkg.go.dev/expvar): whitebox program telemetry all contained in `expvar.go`.
- moderately sized packages that contain one large domain and its multiple responsibilities together:
  - [package `flag`](https://pkg.go.dev/flag): command line flag management all contained in `flag.go`.
- large packages that divide several closely related domains across several files:
  - [package `http`](https://pkg.go.dev/net/http): the core of HTTP: `client.go`, support for HTTP clients; `server.go`, support for HTTP servers; `cookie.go`, cookie management.
  - [package `os`](https://pkg.go.dev/os): cross-platform operating system abstractions: `exec.go`, subprocess management; `file.go`, file management; `tempfile.go`, temporary files.

See also:

- [Test double packages](#test-double-and-helper-packages)
- [Organizing Go Code (Blog Post)](https://go.dev/blog/organizing-go-code)
- [Organizing Go Code (Presentation)](https://talks.golang.org/2014/organizeio.slide)
...
- See also guidance on [error logging](#logging-errors).
...
- [A post by the Go Blog on errors](https://go.dev/blog/errors-are-values)
- [Package errors](https://pkg.go.dev/errors)
- [Package upspin.io/errors](https://pkg.go.dev/upspin.io/errors)
...
- [Error Documentation Conventions](#errors-1)
- [Blog post on error wrapping](https://go.dev/blog/go1.13-errors)
...
Errors can be wrapped with the `%w` verb, or by placing them in a structured error that implements `Unwrap() error` (ex: [`fs.PathError`](https://pkg.go.dev/io/fs#PathError)).
...
- Like [good test failure messages](./decisions.md#useful-test-failures), log messages should clearly express what went wrong and help the maintainer by including relevant information to diagnose the problem.

- Avoid duplication. If you return an error, it's usually better not to log it yourself but rather let the caller handle it. The caller can choose to log the error, or perhaps rate-limit logging using [`rate.Sometimes`](https://pkg.go.dev/golang.org/x/time/rate#Sometimes). Other options include attempting recovery or even [stopping the program](#program-initialization). In any case, giving the caller control helps avoid logspam.
...
As stated in the [decision against panics](./decisions.md#dont-panic), standard error handling should be structured around error return values. Libraries should prefer returning an error to the caller rather than aborting the program, especially for transient errors.
...
- [Handling panics and Run-time Panics](https://go.dev/ref/spec#Handling_panics) in the language specification
...
This section augments the decisions document's [commentary section](./decisions.md#commentary).

Go code that is documented in familiar style is easier to read and less likely to be misused than something misdocumented or not documented at all. Runnable [examples](https://go.dev/blog/examples) show up in Godoc and Code Search and are an excellent way of explaining how to use your code.
...
Document overall error conventions in the [package's documentation](./decisions.md#package-comments) when the behavior is applicable to most errors found in the package:
...
Go features a documentation server. It is recommended to preview the documentation your code produces both before and during the code review process. This helps to validate that the [godoc formatting](./decisions.md#doc-comments) is rendered correctly.
...
- Test files can contain [runnable examples](https://go.dev/blog/examples) that appear attached to the corresponding documentation in godoc:
...
The following declarations use the [zero value](https://go.dev/ref/spec#The_zero_value):
...
A common application of zero value declaration is when using a variable as the output when [unmarshalling](https://pkg.go.dev/encoding/json#Unmarshal):
...
If you need a lock or other field that [must not be copied](./decisions.md#copying) in your struct, you can make it a value type to take advantage of zero value initialization. It does mean that the containing type must now be passed via a pointer and not a value. Methods on the type must take pointer receivers.
...
For map and slice types, if the code is particularly performance sensitive and if you know the sizes in advance, see the [size hints](#size-hints) section.

### Composite literals

The following are [composite literal](https://go.dev/ref/spec#Composite_literals) declarations:
...
In contrast, using composite literals to declare empty or memberless values can be visually noisy compared to [zero-value initialization](#declaring-variables-with-zero-values).
...
**Warning:** Preallocating more memory than you need can waste memory in the fleet or even harm performance. When in doubt, see [GoTip #3: Benchmarking Go Code](https://google.github.io/styleguide/go/index.html#gotip) and default to a [zero initialization](#declaring-variables-with-zero-values) or a [composite literal declaration](#composite-literals).
...
See also Bryan Mills' talk "Rethinking Classical Concurrency Patterns": [slides](https://drive.google.com/file/d/1nPdvhB0PutEJzdCq5ms6UI58dp50fcAN/view) [video](https://www.youtube.com/watch?v=5zXAHh5tJqQ).
...
The recommendations below primarily apply to exported APIs, which are held to a higher standard than unexported ones. These techniques may be unnecessary for your use case. Use your judgment, and balance the principles of [clarity](./guide.md#clarity) and [least mechanism](./guide.md#least-mechanism).
...
See [Rob Pike's original blog post](https://commandcenter.blogspot.com/2014/01/self-referential-functions-and-design.html) and [Dave Cheney's talk](https://dave.cheney.net/2014/10/17/functional-options-for-friendly-apis) for a more in-depth look at how these options can be used.
...
- **Test helpers** are functions that do setup or cleanup tasks. All failures that occur in test helpers are expected to be failures of the environment (not from the code under test) — for example when a test database cannot be started because there are no more free ports on this machine. For functions like these, calling `t.Helper` is often appropriate to mark them as a [test helper](#test-helpers). See [error handling in test helpers](#error-handling-in-test-helpers) for more details.

- **Assertion helpers** are functions that check the correctness of a system and fail the test if an expectation is not met. Assertion helpers are [not considered idiomatic](./decisions.md#assertion-libraries) in Go.

The purpose of a test is to report pass/fail conditions of the code under test. The ideal place to fail a test is within the `Test` function itself, as that ensures that [failure messages](./decisions.md#useful-test-failures) and the test logic are clear.
...
- If inputs are similar, consider unifying them into a [table-driven test](./decisions.md#table-driven-tests) while keeping the logic inlined in the loop. This helps to avoid repetition while keeping the validation and failure in the `Test`.
- If there are multiple callers who need the same validation function but table tests are not suitable (typically because the inputs are not simple enough or the validation is required as part of a sequence of operations), arrange the validation function so that it returns a value (typically an `error`) rather than taking a `testing.T` parameter and using it to fail the test. Use logic within the `Test` to decide whether to fail, and to provide [useful test failures](./decisions.md#useful-test-failures). You can also create test helpers to factor out common boilerplate setup code.
...
**Note:** There is an analogy between test helpers and plain library code. Code in libraries should usually [not panic](./decisions.md#dont-panic) except in rare circumstances; code called from a test should not stop the test unless there is [no point in proceeding](#t-error-vs-t-fatal).
...
In a typical Go test, the test function controls the program flow, and the [no assert](./decisions.md#assertion-libraries) and [test functions](#leave-testing-to-the-test-function) guidance encourages you to keep it that way. This section explains how to author support for these tests in a way that is consistent with Go style.
...
1. Create a new package for the validation behavior, [customarily named](#test-double-and-helper-packages) by appending the word `test` to the package name (for example, `chesstest`).
...
- **Fail fast:** return an error as soon as the implementation violates an invariant.
  ...
  This is the simplest approach, and it works well if the acceptance test is expected to execute quickly. Simple error [sentinels and custom types](https://go.dev/blog/go1.13-errors) can be used easily here, which conversely makes [testing the acceptance test](#use-real-transports) easy.
...
- **Aggregate all failures:** collect all failures, and report them all.
  ...
  This approach resembles the [keep going](./decisions.md#keep-going) guidance in feel and may be preferable if the acceptance test is expected to execute slowly.

  How you aggregate the failures should be dictated by whether you want to give users the ability or yourself the ability to interrogate individual failures (for example, for you to test your acceptance test). Below demonstrates using a [custom error type](https://go.dev/blog/go1.13-errors) that [aggregates errors](https://pkg.go.dev/errors):
...
The acceptance test should honor the [keep going](./decisions.md#keep-going) guidance by not calling `t.Fatal` unless the test detects a broken invariant in the system being exercised.

For example, `t.Fatal` should be reserved for exceptional cases such as [setup failure](#error-handling-in-test-helpers) as usual:
...
This technique can help you create concise, canonical validations. But do not attempt to use it to bypass the [guidance on assertions](./decisions.md#assertion-libraries).
...
**Warning:** It is not always safe to call `t.Fatal` and similar functions. [More details here](#dont-call-t-fatal-from-separate-goroutines).

### Error handling in test helpers

**Note:** This section discusses [test helpers](#test-helpers) in the sense Go uses the term: functions that perform test setup and cleanup, not common assertion facilities. See the [test functions](#leave-testing-to-the-test-function) section for more discussion.

Operations performed by a [test helper](#test-helpers) sometimes fail. For example, setting up a directory with files involves I/O, which can fail. When test helpers fail, their failure often signifies that the test cannot continue, since a setup precondition failed. When this happens, prefer calling one of the `Fatal` functions in the helper:
...
[Test helpers](#test-helpers) usually don't signal failure from new goroutines, and therefore it is all right for them to use `t.Fatal`. If in doubt, call `t.Error` and return instead.
...
If **all tests in the package** require common setup and the **setup requires teardown**, you can use a [custom testmain entrypoint](https://pkg.go.dev/testing#hdr-Main). This can happen if the resource the test cases require is especially expensive to setup, and the cost should be amortized. Typically you have extracted any unrelated tests from the test suite at that point. It is typically only used for [functional tests](https://en.wikipedia.org/wiki/Functional_testing).

Using a custom `TestMain` should not be your first choice due the amount of care that should be taken for correct use. Consider first whether the solution in the [amortizing common test setup](#amortizing-common-test-setup) section or an ordinary [test helper](#test-helpers) is sufficient for your needs.
...
Ideally a test case is [hermetic](https://testing.googleblog.com/2012/10/hermetic-servers.html) between invocations of itself and between other test cases.
...
- [Unit Testing Practices on Public APIs](https://testing.googleblog.com/2008/07/how-to-write-3v1l-untestable-code.html)

Global state has cascading effects on the [health of the Google codebase](https://testing.googleblog.com/2008/07/how-to-write-3v1l-untestable-code.html). Global state should be approached with **extreme scrutiny**.

[Global state comes in several forms](#major-forms-of-package-state-apis), and you can use a few [litmus tests](#litmus-tests) to identify when it is safe.
...
See the [litmus tests](#litmus-tests) to know when these are safe.

- The [service locator pattern](https://en.wikipedia.org/wiki/Service_locator_pattern). See the [first example](#global-state). The service locator pattern itself is not problematic, rather the locator being defined as global.
...
[APIs using the patterns above](#major-forms-of-package-state-apis) are **unsafe** when:
...
- The global state is logically constant ([example](https://pkg.go.dev/net/http#FileServer)).
...
An example of one of these safe situations is [`package image`](https://pkg.go.dev/image) with its [`image.RegisterFormat`](https://pkg.go.dev/image#RegisterFormat) function. Consider the [litmus tests](#litmus-tests) from above applied to a typical decoder, like the one for handling the PNG format:
...
Follow the [litmus tests](#litmus-tests) with these guidelines in such cases:

1. The package must offer clients the ability to create isolated instances of package types [as described above](#global-state).
...
3. This package-level API must only be used by [binary build targets](https://docs.bazel.build/versions/main/be/go.html#go_binary), not [libraries](https://docs.bazel.build/versions/main/be/go.html#go_library), unless the libraries are undertaking a refactoring to support dependency passing. Infrastructure libraries that can be imported by other packages must not rely on package-level state of the packages they import.
