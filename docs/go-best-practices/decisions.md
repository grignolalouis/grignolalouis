...
**Note:** This is part of a series of documents that outline Go Style at Google. This document is **normative but not canonical**, and is subordinate to the core style guide. See the [overview](./index.md) for more information.
...
See the [Overview](./index.md) for the full set of Go Style documents.

The following sections have moved from style decisions to another part of the guide:

- **MixedCaps**: see [guide#mixed-caps](./guide.md#mixedcaps)
...
The conventions around commentary (which include what to comment, what style to use, how to provide runnable examples, etc.) are intended to support the experience of reading the documentation of a public API. See [Effective Go](https://go.dev/doc/effective_go) for more information.
...
Packages should clearly document their intended usage. Try to provide a runnable example; examples show up in Godoc. Runnable examples belong in the test file, not the production source file. See [this example (Godoc, source)](https://pkg.go.dev/time#example-Duration).
...
Exported functions that return errors should return them using the `error` type. Concrete error types are susceptible to subtle bugs: a concrete nil pointer can get wrapped into an interface and thus become a non-nil value (see the [Go FAQ entry on the topic](https://go.dev/doc/faq#nil_error)).
...
**Note:** `log.Fatalf` is not the standard library log. See [#logging](#logging).
...
For more discussion and examples of error handling, see [Effective Go](https://go.dev/doc/effective_go#errors) and [best practices](./best-practices.md).
...
See [Go Tip #1: Line of Sight](https://google.github.io/styleguide/go/index.html#gotip) and TotT: Reduce Code Complexity by Reducing Nesting for more details.
...
See [best practices](./best-practices.md) for a few options for shortening the call sites of functions that would otherwise have many arguments.
...
Do not use `panic` for normal error handling. Instead, use `error` and multiple return values. See the [Effective Go section on errors](https://go.dev/doc/effective_go#errors).
...
Also see [when panic is acceptable](#must-functions).
...
Where `Must` functions are used in a test, they should generally be marked as a test helper and call `t.Fatal` on error (see [error handling in test helpers](#test-helpers) for more considerations of using that).
...
- [When Go programs end](https://go.dev/ref/mem#finalizer)
- [Documentation Conventions: Contexts](#contexts)
...
Go programs in the Google codebase use a variant of the standard `log` package. It has a similar but more powerful interface and interoperates well with internal Google systems. An open source version of this library is available as package [glog](https://pkg.go.dev/github.com/golang/glog), and open source Google projects may use that, but this guide refers to it as `log` throughout.
...
Assertion libraries are libraries that attempt to combine the validation and production of failure messages within a test (though the same pitfalls can apply to other test helpers as well). For more on the distinction between test helpers and assertion libraries, see [best practices](./best-practices.md).
...
- For more on the distinction between test helpers and assertion helpers, see [best practices](./best-practices.md)
- [Go FAQ section on testing frameworks and their opinionated absence](https://go.dev/doc/faq#testing_framework)
...
If your struct needs to be compared for approximate (or equivalent kind of semantic) equality or it contains fields that cannot be compared for equality (e.g., if one of the fields is an `io.Reader`), tweaking a `cmp.Diff` or `cmp.Equal` comparison with `cmpopts` options such as `cmpopts.IgnoreInterfaces` may meet your needs ([example](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#example-IgnoreInterfaces)).
...
**Best practice:** For more discussion about when `t.Fatal` should be used, see [best practices](./best-practices.md).
...
To compute diffs for such values, `cmp.Diff` is preferred, particularly for new tests and new code, but other tools may be used. See [types of equality](#equality-comparison-and-diffs) for guidance regarding the strengths and weaknesses of each function.
...
- When testing whether the outputs of a function always conform to the same set of invariants. For example, [tests for `net.Dial`](https://cs.opensource.google/go/go/+/master:src/net/dial_test.go;l=318;drc=5b606a9d2b7649532fe25b3e8f5e7e97b85e9003).
...
**Note:** The failure messages in this example above fulfill the guidance to [identify the function](#identify-the-function) and [identify the input](#identify-the-input). There's no need to [identify the row numerically](#identifying-the-row).
...
If you pass a `*testing.T`, call `t.Helper` to attribute failures in the test helper to the line where the helper is called. This parameter should come after a [context](#contexts) parameter, if present, and before any remaining parameters.
...
Do not use this pattern when it obscures the connection between a test failure and the conditions that led to it. Specifically, the guidance about [assert libraries](#assertion-libraries) still applies, and `t.Helper` should not be used to implement such libraries.

**Tip:** For more on the distinction between test helpers and assertion helpers, see [best practices](./best-practices.md).
...
The Go standard library provides the [`testing` package](https://pkg.go.dev/testing). This is the only testing framework permitted for Go code in the Google codebase. In particular, [assertion libraries](#assertion-libraries) and third-party testing frameworks are not allowed.

The `testing` package provides a minimal but complete set of functionality for writing good tests:

- Top-level tests
- Benchmarks
- [Runnable examples](https://go.dev/blog/examples)
- Subtests
- Logging
...
- **Local variable initialization with zero value.** `var i int` and `i := 0` are equivalent. See also [initialization best practices](./best-practices.md).
- **Empty composite literal vs. `new` or `make`.** `&File{}` and `new(File)` are equivalent. So are `map[string]bool{}` and `make(map[string]bool)`. See also [composite declaration best practices](./best-practices.md).
- **`got`, `want` argument ordering in `cmp.Diff` calls.** Be locally consistent, and [include a legend in your failure message](#print-diffs).
